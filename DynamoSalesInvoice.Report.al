report 60002 "Dynamo Sales - Invoice"
{
    RDLCLayout = './src/Reports/Layouts/DynamoSalesInvoice.rdlc';
    Caption = 'Dynamo Sales - Invoice';
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem(Header; "Sales Invoice Header")
        {
            ...
            { }
            column(ComapnyFax; CompanyInfo."Fax No.")
            { }
            column(ComapnyTaxNo; CompanyInfo."Tax ID No. -CL-")
            { }
            column(Net_Amt__Authorized__LCY___CL_; "Net Amt. Authorized (LCY) -CL-")
            { }
            column(NetAmountAuthorized; NetAmountAuthorized)
            {
            ...
            { }
            column(UserPhone; Usersetup."Phone No.")
            { }
            ...
            // {
            // }
            column(CompanyAddress1; CompanyInfo.Name + ' ' + CompanyInfo."Name 2")
            {
            }
            column(CompanyAddress2; CompanyInfo.Address + ' ' + CompanyInfo."Address 2")
            {
            }
            column(CompanyAddress3; CompanyInfo.City + ', ' + CompanyInfo.County + ' ' + CompanyInfo."Post Code")
            {
            }
            ...
            dataitem(Line; "Sales Invoice Line")
            {
                ...
                //>> Dynamo
                column(Description_2_Line; "Description 2")
                {

                }
                column(ItemURL_Line; Item."Item URL")
                {
            ...
            begin
                ...
                NetAmountAuthorized := 0;
                EFTTransactions.SetRange("Document No.", Header."No.");
                EFTTransactions.SetRange("Document Type", EFTTransactions."Document Type"::"Posted Invoice");
                EFTTransactions.SetRange("Transaction Status", EFTTransactions."Transaction Status"::Approved);
                EFTTransactions.SetFilter("Method Type", '%1|%2|%3', EFTTransactions."Method Type"::Authorize, EFTTransactions."Method Type"::"Return Authorize", EFTTransactions."Method Type"::"Voice Authorize");

                if EFTTransactions.FindSet() then
                        repeat
                            NetAmountAuthorized += EFTTransactions."Total Amount";
                        until EFTTransactions.Next() = 0;
    ...
    var
        ...
        User: Record User;
        NetAmountAuthorized: Decimal;
        CountryRegion: Record "Country/Region";
        EFTTransactions: Record "EFT Transaction -CL-";
        // Dynamo <<
        SalespersonLbl: Label 'Salesperson';
        CompanyInfoBankAccNoLbl: Label 'Account No.';
        CompanyInfoBankNameLbl: Label 'Bank';
        CompanyInfoGiroNoLbl: Label 'Giro No.';
        CompanyInfoPhoneNoLbl: Label 'Phone';
        CopyLbl: Label 'Copy';
